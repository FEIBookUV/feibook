...
function menu_builder_get_menu_items(){
	global $CONFIG;
	...
	if($items = elgg_get_entities($options)){
		$parents = array();
		...
		foreach($items as $item){
			...
			if(!empty($parent_guid)){
				if($temp = get_entity($parent_guid)){
					if($temp->getSubtype() != MENU_BUILDER_SUBTYPE){
						$parent_guid = 0;
					}
				} else {
					$parent_guid = 0;
			...
			$parents[$parent_guid][] = $item;
		}
			
		$result = menu_builder_sort_menu_items($parents, 0);
	}
...
function menu_builder_menu_item_is_selected($menu_item){
	...
	$item_info = parse_url($menu_item->getURL());
