...
var tockens = '<?php echo elgg_add_action_tokens_to_url("")?>';
...
function addFriend(sessionid, friend){
	$.post("<?php echo $CONFIG->wwwroot; ?>action/elggchat/invite" + tockens + "&chatsession=" + sessionid + "&friend=" + friend, function(){
...
function leaveSession(sessionid){
		...
		$.post("<?php echo $CONFIG->wwwroot; ?>action/elggchat/leave" + tockens + "&chatsession=" + sessionid, function(){
		$("#" + sessionid).remove();
		checkForSessions();
		});
		} 
...
function startSession(friendGUID){
		$.post("<?php echo $CONFIG->wwwroot;?>action/elggchat/create" + tockens + "&invite=" + friendGUID, function(data){
		if(data){
		checkForSessions();
		openSession(data);
		}
...
function notify_new_message(){
		$.sound.play("<?php echo $CONFIG->wwwroot; ?>mod/elggchat/js/sound/soundbip.mp3");	
	}
...
function checkForSessions(firsttime){
		...
		processing = true;
		$.getJSON("<?php echo $CONFIG->wwwroot; ?>action/elggchat/poll" + tockens, function(data){
			...
			if(($("#" + sessionid).hasClass("elggchat_session_new_messages"))){	
			notify_new_message();
			}
			...
			if(input != ""){
			$.post("<?php echo $CONFIG->wwwroot;?>action/elggchat/post_message" + tockens, $(this).serialize(), function(data){
			checkForSessions();
			});
			}
			// empty input field
			...
			if(typeof(data.friends) != "undefined"){
				...
				var show_offline = "<?php echo get_plugin_usersetting("show_offline_user",0,"elggchat")?>";
