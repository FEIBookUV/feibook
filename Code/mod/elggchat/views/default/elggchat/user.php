...
if(!empty($user) && $user instanceof ElggUser){
	...
	if(empty($iconSizeforchat) || !in_array($iconSizeforchat, array("tiny", "small", "medium", "large", "profile"))){
		$iconSizeforchat = "small";
	}
	...
	if($icon){
	$result .= "<td><div style='margin-bottom:5px;'><img src='" . $user->getIcon($iconSize) . "' alt='" . $user->name . "' /></td>";
              
	 }
	
	if($link){
			$result .= "<td class='chatmemberinfo' style='padding-left:5px;'><a href='" . $user->getUrl() . "' title='" . $user->name . "' rel='" . $user->guid . "'>" . $user->name . "</a></td>";
	} else { 
			$result .= "<td class='chatmemberinfo'  style='padding-left:5px;'>". $user->name . "</div></td>";
	}               
