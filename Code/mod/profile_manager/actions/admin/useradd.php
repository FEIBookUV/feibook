						...
						if(!empty($use_default_access)){
							...
							if (is_array($metadata_value)) {
								...
								foreach($metadata_value as $interval) {
									...
									create_metadata($new_user->guid, $metadata_name, $interval, 'text', $new_user->guid, ACCESS_DEFAULT, $multiple);
								}
							} else {
								create_metadata($new_user->guid, $metadata_name, $metadata_value, 'text', $new_user->guid, ACCESS_DEFAULT);
							}	
